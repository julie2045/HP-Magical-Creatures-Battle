...
class Game {
  ...
  void startGame() {
    ...
    System.out
        .println("Play against yourself/another person and choose the best mythical creature to win the battle! ");
    ...
    // environment if and else statements --> increase strength depening on their
    // best environment
    System.out.println();
    System.out.println(
        "Pick an environment by typing in the corresponding number! (environment may or may not increase strength depening on the creature): ");
  ...
  boolean isGameOver() {
    if (this.player1.isDefeated()) {
      System.out
          .println(String.format("%s is defeated! %s won the fight!", this.player1.species(), this.player2.species()));
      return true;
    } else if (this.player2.isDefeated()) {
      System.out
          .println(String.format("%s is defeated! %s won the fight!", this.player2.species(), this.player1.species()));
      return true;
    }
    return false;
  }
  ...
  void updateGameStatus(int round) {
    ...
    System.out.println(String.format("%s: %d hp | %s: %d hp", this.player1.species(), this.player1.getHealth(),
        this.player2.species(), this.player2.getHealth()));
    System.out.println("-".repeat(70));
